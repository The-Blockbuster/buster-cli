...
const confirmDelete = function () {
    return askYesNoQuestion(
        chalk`{bold.white This method will delete your account. Beneficiary account must already be initialized in order to transfer all Boom tokens or these will be lost. Make sure to send all fungible tokens or NFTs that you own to the beneficiary account prior to deleting, as this method will only transfer BOOM tokens. Do you want to proceed? {bold.green (y/n) }}`,
        false);
};
...
exports.deploy = async function (options) {
    ...
    if (options.force || await checkExistingContract(prevCodeHash)) {
        ...
        if (options.initFunction) {
            if (!options.initArgs) {
                console.error('Must add initialization arguments.\nExample: buster deploy --accountId buster.testnet --initFunction "new" --initArgs \'{"key": "value"}\'');
                await eventtracking.track(eventtracking.EVENT_ID_DEPLOY_END, {
                    success: false,
                    error: 'Must add initialization arguments'
                }, options);
                process.exit(1);
            }
        ...
        await eventtracking.track(eventtracking.EVENT_ID_DEPLOY_END, {
            success: true,
            code_hash: codeHash,
            is_same_contract: prevCodeHash === codeHash,
            contract_id: options.accountId
        }, options);
...
exports.login = async function (options) {
    ...
    if (!options.walletUrl) {
        ...
        await eventtracking.track(eventtracking.EVENT_ID_LOGIN_END, {
            success: true,
            login_is_not_needed: true
        }, options);
    } else {
        const newUrl = new URL(options.walletUrl + '/login/');
        const referrer = 'BUSTER CLI';
        newUrl.searchParams.set('referrer', referrer);
        const keyPair = await KeyPair.fromRandom('ed25519');
        newUrl.searchParams.set('public_key', keyPair.getPublicKey());

        console.log(chalk`\n{bold.yellow Please authorize BUSTER CLI} on at least one of your accounts.`);
        ...
        const getAccountFromConsole = async () => {
            return await new Promise((resolve) => {
                rl.question(
                    ...
                    chalk`Which account did you authorize for use with BUSTER CLI?\n` +
...
exports.deleteAccount = async function (options) {
    ...
    try {
        ...
        if (e.type === 'AccountDoesNotExist') {
            console.error(`Beneficiary account ${options.beneficiaryId} does not exist. Please create the account to transfer Boom tokens.`);
            return;
        } else {
            throw e;
        }
...
exports.sendMoney = async function (options) {
    ...
    console.log(`Sending ${options.amount} BOOM to ${options.receiver} from ${options.sender}`);
    const near = await connect(options);
...
exports.b_sendMoney = async function (options) {
    await checkCredentials(options.sender, options.networkId, options.keyStore);
    console.log(`Sending ${options.amount} BOOM to ${options.receiver} from ${options.sender}`);
    const near = await connect(options);
    const account = await near.account(options.sender);
