...
const js_deploy = {
    ...
    builder: (yargs) => yargs
        ...
        .option('depositYocto', {
            desc: 'Deposit (in Yocto BOOM) to maintain the contract storage on the enclave',
            type: 'string',
            default: null,
        })
        .option('accountId', {
...
async function dev_deploy(options) {
    if (options.networkId === 'mainnet') {
        throw Error('MainNet doesn\'t support dev-deploy. Use export BUSTER_ENV=testnet to switch to TestNet');
    }
...
async function call(options) {
    ...
    console.log(`Scheduling a call in JSVM[${jsvmId}]: ${options.contractId}.${options.methodName}(${options.args || ''})` +
        (deposit && deposit != '0' ? ` with attached ${utils.format.formatNearAmount(deposit)} BOOM` : ''));
