    ...
    if (!(await keyStore.getKey(networkId, accountId))) {
        ...
        if (defaultKeys && networkId === 'testnet') {
            ...
            if (answer) {
                ...
                throw console.log(
                    chalk`{bold.white Please relocate credentials found in [ {bold.blue default} ] directory to [ {bold.blue testnet} ] directory using {bold.yellow buster login}.}`
                );
            }
        } else if (testnetKeys && networkId === 'default') {
            console.log(
                chalk`{bold.white Key found in [ testnet ] folder but your command is configured to use {bold.yellow deprecated} [ default ] network. Please upgrade buster-cli or change your project\'s config file. }`
            );
        }
    }
};
